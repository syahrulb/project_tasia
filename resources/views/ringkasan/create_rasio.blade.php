@extends('layouts.layout')
@section('Title')
    CES Jasa Persewaan
@endsection
@section('Content')
    <!-- Content Wrapper. Contains page content -->
    <div class="content-wrapper">
        <!-- Content Header (Page header) -->
        <section class="content-header">
            <h1>
                MASTER RASIO
                <br>
            </h1>

        </section>

        <!-- Main content -->
        <section class="content">
            <!-- Main row -->
            <div class="row">
                <!-- Left col -->
                <div class="col-md-6">
                    <!-- iCheck -->
                    <div class="box">
                        <div class="box-header">
                            <h4 class="box-title"> Create Rasio </h4>
                        </div>
                        <div class="box-body">
                            <form method="post" action="{{ action('PengaturanRasioController@storerasio') }}">
                                <div class="row">
                                    <!-- ./col -->
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            @csrf
                                            <label> Nama Rasio :</label>
                                            <div class="input-group">
                                                <input type="text" name="nama_rasio" class="form-control pull-right"
                                                       id="id_akun">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label> Jenis Rasio :</label>
                                            <div class="input-group">
                                                <select name="jenis_rasio" class="form-control" style="width: 100%;">
                                                    <option disabled selected="selected"> -- Pilih Jenis Rasio --
                                                    </option>
                                                    @foreach($jenis_rasios as $jenis)
                                                        <option
                                                            value="{{$jenis->id_jenis_rasio}}">{{$jenis->jenis_rasio}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label> Rumus Rasio </label>
                                            <div class="input-group">
                                                <div class="col-md-5">
                                                    <div class="form-group">
                                                        <label>Kelompok 1</label>
                                                        <select name="id_kelompok[0]" class="form-control select2"
                                                                style="width: 100%;">
                                                            <option disabled selected="selected">-- Pilih Kelompok --
                                                            </option>
                                                            @foreach($pengelompokans as $kelompok)
                                                                <option
                                                                    value="{{$kelompok->id_kelompok}}">{{$kelompok->kegunaan_akun}}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                </div>
                                                <div class="col-md-1">
                                                    <h1>:</h1>
                                                </div>
                                                <div class="col-md-5">
                                                    <div class="form-group">
                                                        <label>Kelompok 2</label>
                                                        <select name="id_kelompok[1]" class="form-control select2"
                                                                style="width: 100%;">
                                                            <option selected="selected">-- Pilih Kelompok --</option>
                                                            @foreach($pengelompokans as $kelompok)
                                                                <option
                                                                    value="{{$kelompok->id_kelompok}}">{{$kelompok->kegunaan_akun}}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>

                                    <!-- ./col -->
                                </div>
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="table-wrapper">
                                            <div class="table-title">
                                                <div class="row">
                                                    <div class="col-sm-8"><h2>Add <b>Kriteria</b></h2></div>
                                                    <div class="col-sm-4">
                                                        <button type="button" class="btn btn-info add-new"><i
                                                                class="fa fa-plus"></i> Add New
                                                        </button>
                                                    </div>
                                                </div>
                                            </div>
                                            <table id="tableKriteria" class="table table-bordered">
                                                <thead>
                                                <tr>
                                                    <th>Operator</th>
                                                    <th>Nilai Batas</th>
                                                    <th>Kriteria</th>
                                                    <th>Actions</th>
                                                </tr>
                                                </thead>
                                                <tbody>
                                                <tr>

                                                    <td>
                                                        <select name="kriteria_rasio[0][operator]" class="form-control"
                                                                style="width: 100%;">
                                                            <option selected="selected" disabled> -- Pilih Operator --
                                                            </option>
                                                            <option value="<"><</option>
                                                            <option value=">">></option>
                                                            <option value="<="><=</option>
                                                            <option value=">=">>=</option>
                                                            <option value="=">=</option>
                                                        </select>
                                                    </td>
                                                    <td><input type="number" step="any" class="form-control"
                                                               name="kriteria_rasio[0][nilai_batas]"></td>
                                                    <td><input type="text" class="form-control"
                                                               name="kriteria_rasio[0][kriteria]"></td>
                                                    <td>
                                                        <a class="delete btn btn-danger" style="color: white"
                                                           title="Delete" data-toggle="tooltip">-</a>
                                                    </td>
                                                </tr>
                                                </tbody>
                                            </table>
                                        </div>
                                    </div>

                                </div>
                        </div>
                        <div class="box-footer">
                            <button type="submit" class="btn btn-primary pull-right">Submit</button>
                        </div>
                        </form>
                    </div>
                </div>
            </div>
        </section>
    </div>
@endsection
@section('custom_script')
    <script>
        $(document).ready(function () {
            $('.select2').select2();
            $('[data-toggle="tooltip"]').tooltip();
            var actions = $("#tableKriteria td:last-child").html();
            console.log('actions', actions);
            // Append table with add row form on add new button click
            $(".add-new").click(function () {
                var index = $("table tbody tr:last-child").index();
                console.log(index);
                var row = '<tr>' +
                    '<td><select name="kriteria_rasio[' + (index + 1) + '][operator]" class="form-control" style="width: 100%;"> <option selected="selected" disabled> -- Pilih Operator -- </option><option value="<"><</option><option value=">">></option><option value="<="><=</option><option value=">=">>=</option><option value="=">=</option></select></td>' +
                    '<td><input type="number" step="any" class="form-control" name="kriteria_rasio[' + (index + 1) + '][nilai_batas]"></td>' +
                    '<td><input type="text" class="form-control" name="kriteria_rasio[' + (index + 1) + '][kriteria]"></td>' +
                    '<td>' + actions + '</td>' +
                    '</tr>';
                $("table").append(row);
                $("table tbody tr").eq(index + 1).find(".add, .edit").toggle();
                $('[data-toggle="tooltip"]').tooltip();
            });
            // Add row on add button click
            // Delete row on delete button click
            $(document).on("click", ".delete", function () {
                $(this).parents("tr").remove();
                $(".add-new").removeAttr("disabled");
            });
        });
    </script>
@endsection
